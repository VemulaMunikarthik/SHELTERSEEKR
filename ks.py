...
def knapsack_problem(accommodations, budget, amenities, prox):
  ...
  for i in range(1, n + 1):
    for j in range(1, budget + 1):
      name, distance, rent, amenities = accommodations[
        i - 1]  # Update the order of unpacking
      if rent <= j:
        included_score = calculate_score(rent, distance, amenities, budget,
                                         prox) + dp[i - 1][j - rent]
...
def main(acc, budget, amenities, prox):  #from maps get_data
  ...
  if best_accommodations == []:
    ...
    d = {
      accommodation[0]: (accommodation[2], accommodation[1], accommodation[3])
    }
    return d
