...
def find_nearby_hostels(input_location, budget, amenity):
  ...
  places_url = f"https://maps.googleapis.com/maps/api/place/nearbysearch/json?location={latitude},{longitude}&radius=5000&type=lodging&keyword=hostels%20dorms%20pgroom&key={GOOGLE_API_KEY}&limit=6"
  ...
  for hostel in hostels:
    ...
    hostel_coordinates.append((hostel['geometry']['location']['lat'],
...
def get_data(input_location, budget, amenities_count):
  ...
  for hostel in range(len(hostels)):
    d[hostels[hostel]['name']] = (distclear(hostels[hostel]['distance']),
                                  str(hostels[hostel]['rent']),
                                  hostels[hostel]['amen'],
                                  hostels[hostel]['link'])
  return d
